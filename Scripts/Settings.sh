...
# ========== 彻底移除 ddnsto（删除源码 + 禁用配置） ==========
rm -rf ./feeds/nas/network/services/ddnsto 2>/dev/null
sed -i '/CONFIG_PACKAGE_ddnsto/d' .config
sed -i '/CONFIG_PACKAGE_luci-app-ddnsto/d' .config
echo "# CONFIG_PACKAGE_ddnsto is not set" >> .config
echo "# CONFIG_PACKAGE_luci-app-ddnsto is not set" >> .config
echo "ddnsto removed and disabled"
